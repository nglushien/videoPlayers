        ...
        * until it has looped over the entire playlist ids array ****/
        ...
        buildPlaylistSelector = function() {
          ...
          ****/
          ...
          if(obj.playlistsData.length === 1){
            $("#BCL_playlistSelectorDropDown li").filter(":first").trigger("click");
          } else {

            var numOfPlaylists = obj.playlistsData.length;
            ...
            var p1 = obj.playlistsData[0].videos;
            var p2 = obj.playlistsData[1].videos;
            var p3 = obj.playlistsData[2].videos;
            var urlHash = window.location.hash.substring(1);

            jQuery.each(p1, function(i, val){
              var ng1 = val.id + "";
              if(urlHash === ng1 || window.location.hash === ''){
                $("#BCL_playlistSelectorDropDown li").filter(":first").trigger("click");
              }
            });

            jQuery.each(p2, function(i, val){
              var ng2 = val.id + "";
              if(urlHash === ng2){
                $("#BCL_playlistSelectorDropDown li").filter(function(index){return index === 1;}).trigger("click");
              }
            });

            jQuery.each(p3, function(i, val){
              var ng3 = val.id + "";
              if(urlHash === ng3){
                $("#BCL_playlistSelectorDropDown li").filter(function(index){return index === 2;}).trigger("click");
              }
            });
          }
        ...
        return {
          ...
          onTemplateReady : function (evt) {
            ...
            videoPlayer.getCurrentRendition(function(renditionDTO) {
              ...
              document.getElementById("playlist_player").style.paddingBottom = newPercentage;
