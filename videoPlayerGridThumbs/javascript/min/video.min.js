 /******** Any scripts specific to page samples should go here *********/
      ...
      var BCLS = (function() {
        var player,
            ...
            playlistTemplate = "{{#videos}}<div class=\"playlist-item\" data-id=\"{{id}}\"><p class=\"duration\">{{length}}</p><img height=\"40\" width=\"72\" src=\"{{videoStillURL}}\"/><h6>{{displayName}}</h6></div>{{/videos}}",
              template,
              data,
              results,
              experienceModule;
          ...
          ****/
          $("#BCL_playlistSelectorDropDown li").on("click", function(evt) {
            ...
            $('.playlist-btn').on('click',function(){
              if($('#BCL_playlist div').hasClass('playlist-item-selected')){
                $('#BCL_playlist').scrollTop($('.playlist-item-selected').position().top - $('.playlist-item-selected').height() + $('#BCL_playlist').scrollTop());
              } else {
